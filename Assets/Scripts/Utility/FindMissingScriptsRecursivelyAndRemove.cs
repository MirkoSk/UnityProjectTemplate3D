...
namespace FLGCoreEditor.Utilities
{
    public class FindMissingScriptsRecursivelyAndRemove : EditorWindow
    {
        ...
        public void OnGUI()
        {
            ...
            EditorGUILayout.TextField("{0} GameObjects Selected", _goCount.ToString());
            if (_goCount > 0) EditorGUILayout.TextField("{0} Components", _componentsCount.ToString());
            if (_goCount > 0) EditorGUILayout.TextField("{0} Deleted", _missingCount.ToString());
        }
